...
struct HyperParameters {
    HyperParameters(int epochs = 10, int batchSize = 8, float learningRate = 0.01,
            ...
            :
            _epochs(epochs), _batchsize(batchSize), _learningRate(learningRate), _optimizer(optimizer), _beta1(beta1),
            _beta2(beta2) {}
...
struct DataSet {
    ...
    validationSamples, std::vector<Matrix> validationLabels)
            : _trainingSamples(trainingSamples), _trainingLabels(trainingLabels), _validationSamples(validationSamples),
