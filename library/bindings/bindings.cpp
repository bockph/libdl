 ...
 */
	...
	py::class_<HyperParameters>(m, "HyperParameters")
			.def(py::init<int, int, float, Optimizer, float, float>(), py::arg("epochs") = 10, py::arg("batchSize") = 8, py::arg("learningRate") = 0.01, py::arg("optimizer") = Optimizer::Adam, py::arg("beta1") = 0.9, py::arg("beta2") = 0.999)
			.def("toString", &HyperParameters::toString);
	py::class_<Graph, std::shared_ptr<Graph>>(m, "Graph").def(py::init<HyperParameters>());
	/*
	 * Definition NeuralNetwor Interface
	 */
	py::class_<NeuralNetwork>(m, "NeuralNetwork").def(py::init<const std::shared_ptr<Graph>, const std::shared_ptr<InputLayer>, const std::shared_ptr<LossLayer>, const HyperParameters>())
			...
			.def("setParams", &NeuralNetwork::setParams)
	 ...
	 * Definition Utils
	 */
	py::class_<LegoDataLoader>(m, "LegoDataLoader").def(py::init<>())
