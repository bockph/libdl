...
//            std::cout<<test<<std::endl;
            ...
            unsigned error = lodepng::decode(image,width,height,test);
...
//    std::cout<<data[0]<<std::endl;
    ...
    std::cout<<data[0].size()<<std::endl;
        for(int i = 200;i<data[0].size()-4;i++){
            if(data[0][i]!=0){
                std::cout<<"Data:"<<data[0][i]<<","<<data[0][i+1]<<","<<data[0][i+2]<<","<<data[0][i+3]<<std::endl;
                i+=3;
            }
