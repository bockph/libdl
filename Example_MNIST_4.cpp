...
int main() {
    ...
    mnist::MNIST_dataset<std::vector, std::vector<uint8_t>, uint8_t> dataset =
            mnist::read_dataset<std::vector, std::vector, uint8_t, uint8_t>(MNIST_DATA_LOCATION,10);
    ...
    filter1 = generateRandomMatrix(-0.2,0.25,8,5*5);
...
//convolutional Layer 2
    ...
    filter2 = generateRandomMatrix(0.,0.025,8,5*5*8);
...
//Dense Layer 1
    ...
    Eigen::MatrixXf mW1 = generateRandomMatrix(0., 1., out3DimSQ, 128);
   
    auto W1 = std::make_shared<Weight>(mW1);
    //Bias
    Eigen::MatrixXf b3 = Eigen::MatrixXf::Zero(1,128);
...
//Dense Layer 2
    ...
    Eigen::MatrixXf mW2 = generateRandomMatrix(0., 1.,128, 10 );
...
//    session.run();
    ...
    for( int input =0;input<10;input++){
        Eigen::Matrix<unsigned char,1,784> tmp(dataset.training_images.at(input).data());
        Eigen::MatrixXf tmp2 = tmp.cast<float>();
        img.block(0,0,1,784)=tmp2;
        Eigen::MatrixXf C(batch_size,10);
        C.setZero();
        C(0,dataset.training_labels.at(input))=1;
        CE->setInputB( std::make_shared<Placeholder>(C,0,0));
        X->setInputA(std::make_shared<Placeholder>(img,28,1));
        for (int i = 0; i < 8; i++) {
            session.run();
    ...
    for( int input =0;input<10;input++){
        ...
        CN->setInputA( std::make_shared<Placeholder>(C,0,0));
        X->setInputA(std::make_shared<Placeholder>(img,28,1));
        std::cout<<"Input:"<<input<<std::endl;
            session.run();
        soft->getForward().block(0,0,1,10).maxCoeff(&maxRow,&maxCol);
        int p = maxCol;
        C.block(0,0,1,10).maxCoeff(&maxRow,&maxCol);
        int A = maxCol;
            std::cout << "LOSS:\n" << CE->getForward()(0,0) << std::endl;

            std::cout << "Output:\n" << p <<"Should be:"<<A<< std::endl;
